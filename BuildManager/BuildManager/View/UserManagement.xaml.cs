...
namespace BuildManager.View
{
    ...
    /// </summary>
    public partial class UserManagement : DbTableModifyBase
    {
        UserManagementViewModel vm = new UserManagementViewModel("User Information");

        private string LastNameColumn = "LastName";

        public UserManagement(MainWindow window) : base()
        {
            this.mainWindow = window;
            base.viewModel = vm;
            this.ColumnNameForEdit = "FirstName";
            InitializeComponent();
        ...
        protected void SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                this.vm.CurrentItemToEdit = e.AddedItems[0] as User;
            }
            else
            {
        ...
        protected override void CellEditEnding(object sender, DataGridCellEditEndingEventArgs e)
        {
            ...
            foreach (DataGridCellInfo cellInfo in datagrid.SelectedCells)
            {
                ...
                if (cellInfo.Column.Header.ToString() == this.ColumnNameForEdit ||
                    cellInfo.Column.Header.ToString() == LastNameColumn)
                {
                    if (gridCell != null)
                    {
                        ...
                        if (tb != null)
                        {
                            ...
                            if ( cellInfo.Column.Header.ToString() == LastNameColumn)
                            {
                                this.vm.ModifyItemLastName(tb.Text);
                            }
                        }
                    }
                }
